...
'''
git code: https://github.com/Lsan2401/RMSIN
'''
...
class Attention(torch.nn.Module):
    def __init__(self, dim, img_shape, att_shape, key_dim=32, num_heads=8, attn_ratio=2, activation=torch.nn.Hardswish):
        ...
        self.dh = int(attn_ratio * key_dim) * num_heads # 512
        self.attn_ratio = attn_ratio
        h = self.dh + nh_kd * 2 # 1024
    ...
    def forward(self, x):  # x (B,N,C)
        B, N, C = x.shape # [2, 314, 480]
        qkv = self.qkv(x) # [2, 31, 1024]
        q, k, v = qkv.view(B, N, self.num_heads, -
                           1).split([self.key_dim, self.key_dim, self.d], dim=3) # torch.Size([2, 314, 8, 32]) torch.Size([2, 314, 8, 32]) torch.Size([2, 314, 8, 64])
        q = q.permute(0, 2, 1, 3)
        k = k.permute(0, 2, 1, 3)
        v = v.permute(0, 2, 1, 3)

        v0 = v[:, :, :self.img_shape, :] # [2, 8, 225, 64]

        v0 = v0.reshape(B, self.dh, int(self.img_shape ** 0.5), -1) # [2, 512, 15, 15]
...
class CrossScaleAttention(nn.Module):
    def __init__(self, dim, img_shape=64, att_shape=89):
        super().__init__()
    ...
    def forward(self, x):
        ...
        attn = torch.cat((x0, x1, x2), dim=2).permute(0, 2, 1) # [2, 314, 480]
        attn = self.attention(attn)
        attn = attn.permute(0, 2, 1).contiguous().view(x0.shape[0], x0.shape[1], 8, 8)
...
class CIM(nn.Module):
    ...
    def forward(self, input):  # [B, C, H, W]
        out = self.pool(input) # multi -scale에서 가장 아래 사이즈로 만들어주고 합치기
        out = self.down_channel(out) # 2, 480, 15, 15
...
if __name__ == '__main__':
    ...
    # vmunet Encoder features
    model = CIM(1440,channels=[96, 192, 384, 768])
    x1 = torch.randn(2, 96, 64, 64)
    x2 = torch.randn(2, 192, 32, 32)
    x3 = torch.randn(2, 384, 16, 16)
    x4 = torch.randn(2, 768, 8, 8)
