from vmamba_convffn import VSSM
...
# ChannelAttn, SpatialAttn :: 참고 자료 https://kmhana.tistory.com/21
...
class VMUNetV2(nn.Module):
    ...
    def forward(self, x):
        seg_outs = []
        if x.size()[1] == 1: # gray 이면 3채널로 바꿔줌
            x = x.repeat(1,3,1,1)
        f1, f2, f3, f4 = self.vmunet(x) #  f1 [2, 64, 64, 96]  f3  [2, 8, 8, 768]  [b h w c]
        
        # b h w c --> b c h w
        f1 = f1.permute(0, 3, 1, 2) # f1 [2, 96, 64, 64]
        f2 = f2.permute(0, 3, 1, 2) # f2 [2, 192, 32, 32]
        f3 = f3.permute(0, 3, 1, 2) # f3 [2, 384, 16, 16]
        f4 = f4.permute(0, 3, 1, 2) # f4 [2, 768, 8, 8]

        f1, f2, f3, f4 = self.cim(tuple([f1, f2, f3, f4]))
        # use sdi  
        f1 = self.ca_1(f1) * f1
        f1 = self.sa_1(f1) * f1
...
if __name__ == '__main__':
    ...
    predict = model(x)
    print(predict.shape)  #  deep_supervision true   predict[0] [2, 1, 256, 256] , predict[1] [2, 1, 128, 128]
